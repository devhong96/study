...
>
>    → **답:** 둘 다 호출 스레드가 응답까지 블로킹돼 스레드 점유 관점에선 사실상 같다. 오히려 이벤트 루프 스레드에서 `.block()`을 부르면 데드락 위험이 있어, 논블로킹 이점을 못 살릴 바엔 이득이 거의 없다.
> 2. 이벤트 루프는 "단일 스레드 비동기"인데, 그 안의 한 작업이 블로킹하면 전체가 멈추는 이유는?
>
>    → **답:** 단일 스레드가 준비된 이벤트를 순차 처리하는 구조라, 한 콜백이 CPU 연산·블로킹 I/O로 스레드를 붙잡으면 그동안 다른 이벤트를 전혀 못 꺼낸다. 그래서 무거운 작업은 별도 워커 풀로 넘겨야 한다.
> 3. 가상 스레드는 "블로킹처럼 쓰는데 논블로킹 효율"을 어떻게 달성하나? (캐리어 스레드 언마운트)
>
>    → **답:** 가상 스레드가 블로킹 호출을 만나면 JVM이 그 스레드를 **캐리어(OS) 스레드에서 언마운트**하고 캐리어는 다른 가상 스레드를 태운다. OS 스레드는 안 막히므로 동기처럼 짜도 소수 캐리어로 수만 작업을 처리한다.
