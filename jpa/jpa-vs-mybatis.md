...
>    → **답:** QueryDSL은 타입안전 동적 조건·조인·DTO 프로젝션 등 대부분의 조회를 커버한다. 윈도우 함수·재귀 CTE·DB 전용 힌트·초고난도 튜닝처럼 JPQL/QueryDSL로 표현이 버거운 쿼리에서 네이티브 SQL이나 MyBatis로 내려간다.
> 2. 혼용 시 **트랜잭션·영속성 컨텍스트 경계**를 어떻게 관리하나? (MyBatis로 UPDATE한 걸 JPA 영속성 컨텍스트가 모르는 stale 문제)
>    → **답:** 같은 트랜잭션에서 MyBatis가 UPDATE해도 JPA 1차 캐시는 그 변경을 몰라 옛 엔티티를 반환할 수 있다. 같은 트랜잭션·커넥션을 공유하도록 묶고, 필요하면 `flush()`/`clear()`로 컨텍스트를 동기화한 뒤 재조회한다.
...
>    → **답:** `hibernate.default_batch_fetch_size`(예: 100)를 전역으로 켜 지연로딩을 IN 절로 묶고, 목록·조회 API는 fetch join/`@EntityGraph`나 DTO 프로젝션으로 처음부터 필요한 걸 한 번에 가져온다. "엔티티=영속, DTO=조회" 원칙.
