...
ㄴ> ThreadLocal은 내부적으로 어떻게 값을 저장하나요?
ㄴ> ThreadLocal.remove()를 호출하지 않으면 어떤 문제가 생기나요?
ㄴ> MDC는 ThreadLocal과 어떤 관계가 있나요?
ㄴ> @Async나 별도 Executor로 넘어가면 TraceId는 유지되나요?
ㄴ> WebFlux 같은 reactive 환경에서는 왜 ThreadLocal 기반 로깅이 어려울까요?
