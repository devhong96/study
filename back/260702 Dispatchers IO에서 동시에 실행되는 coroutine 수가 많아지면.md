...
ㄴ> Coroutine은 가벼운데 왜 thread pool 고갈 문제가 생길 수 있나요?
ㄴ> Dispatchers.IO와 Dispatchers.Default는 어떤 차이가 있나요?
ㄴ> DB 작업을 coroutine으로 많이 날리면 어떤 문제가 생길 수 있나요?
ㄴ> Coroutine에서 backpressure는 어떻게 구현할 수 있을까요?
ㄴ> 외부 API 호출을 coroutine으로 병렬 처리할 때 동시성 제한은 어떻게 둘 수 있나요?
