...
→ **용도와 상한 정책이 다르다 — Default는 CPU 바운드용(≈코어 수), IO는 blocking 대기용(기본 max(64, 코어 수)).**
...
→ **스레드(64)보다 커넥션 풀(예: HikariCP 10)이 먼저 고갈된다.**
...
→ **suspend 자체가 자연스러운 backpressure 수단이다.**
...
→ **Semaphore로 동시 개수를, rate limiter로 초당 개수를 제한한다 — 둘은 별개의 제약이다.**
```kotlin
