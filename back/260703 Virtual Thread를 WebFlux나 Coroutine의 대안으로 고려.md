...
**A.** **이벤트 루프 스레드는 소수·유한한데(Reactor Netty 기본 워커 ≈ max(코어,4)), JPA/JDBC가 그 스레드를 쿼리 응답까지 잡아둔다 — 소수 요청만으로 전체 서버가 멈추는 구조적 최악.**
...
**Q. VT의 스케줄러는 무엇이고 어떻게 동작하나?**

**A.** 전용 ForkJoinPool(work-stealing 방식, 캐리어 수 기본 ≈ 코어 수)이 VT를 carrier에 배정한다. work-stealing이라 특정 carrier가 놀면 다른 큐의 VT를 훔쳐와 실행한다 — CPU를 놀리지 않는 구조라는 점에서 Dispatchers.Default와 같은 계열이다.

**Q. pinning은 실제로 어떻게 진단하나?**

**A.** JVM 옵션 `-Djdk.tracePinnedThreads=full`(21~23)로 pinning 발생 시 스택을 출력하거나, JFR의 `jdk.VirtualThreadPinned` 이벤트로 수집한다. "pinning이 있는지 모른 채 켜는 것"이 도입 실패의 전형이라 사전 측정이 중요하다.

**Q. VT 시대에도 여전히 풀링이 필요한 자원은?**

**A.** 스레드는 싸졌지만 DB 커넥션, 파일 핸들, 외부 API 쿼터 같은 하위 자원은 여전히 유한하다. 즉 "스레드 풀"은 사라져도 "커넥션 풀 + 세마포어"는 남는다 — 풀링의 대상이 스레드에서 진짜 희소 자원으로 이동했다고 정리할 수 있다.
