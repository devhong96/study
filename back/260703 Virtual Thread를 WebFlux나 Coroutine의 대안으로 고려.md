...
**Q. Virtual Thread를 쓰면 모든 blocking 문제가 해결되나요?**
→ **아니오 — 해결되는 것은 "blocking 대기가 플랫폼 스레드를 낭비하는 문제"뿐이다.**
...
→ **이벤트 루프 스레드는 코어 수만큼밖에 없는데, JPA/JDBC가 그 스레드를 쿼리 응답까지 잡아둔다 — 소수 요청만으로 전체 서버가 멈추는 구조적 최악.**
...
→ **PT는 OS 스레드와 1:1(스택 미리 확보, 통상 1MB 안팎, 현실 상한 수천 개), VT는 JVM이 스케줄링하는 경량 스레드(스택을 힙에 작게 시작, 수십만 개 가능).**
...
→ **그 루프가 감시하던 수천 커넥션의 이벤트 처리가 전부 지연·정지된다.**
...
→ **Boot 3.2+에서 `spring.threads.virtual.enabled=true`로 켜되, 네 가지를 점검해야 한다.**
