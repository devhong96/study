...
ㄴ> Virtual Thread를 쓰면 모든 blocking 문제가 해결되나요?
ㄴ> WebFlux를 쓰면서 JPA를 그대로 사용하면 어떤 문제가 생기나요?
ㄴ> Virtual Thread와 Platform Thread의 차이는 무엇인가요?
ㄴ> Event Loop에서 blocking 작업이 발생하면 어떤 문제가 생기나요?
ㄴ> 기존 Spring MVC 애플리케이션에서 Virtual Thread를 도입할 때 주의할 점은 무엇인가요?
