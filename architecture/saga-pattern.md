...
1. 보상이 불가능한 단계(예: 되돌릴 수 없는 이메일 발송, 최종 확정)를 사가에 넣어야 하면 어디에 배치하나?

   → **답:** 보상 불가능·재시도만 가능한 단계는 사가 뒤쪽에 몰아, 그 앞에 "이후로는 전진만(go-forward)"인 기준점(pivot transaction)을 둔다. 그 지점 이후 실패는 보상 대신 재시도로 완주시킨다.
2. 결제 `charge` 호출 후 네트워크로 응답만 유실되면(성공/실패 불명) 그냥 보상(환불)해도 되나?

   → **답:** 안 된다 — 실제로 승인됐을 수 있어 정상 결제를 환불하거나 이중 청구할 위험이 있다. 상태 조회 API로 실제 결과를 확인하거나 멱등 키로 안전하게 재시도해 판정한 뒤 결정한다.
3. 코레오그래피에서 `@TransactionalEventListener`(AFTER_COMMIT)로 다음/보상 이벤트를 발행하는데, 커밋 후 그 발행 자체가 실패하면?

   → **답:** AFTER_COMMIT은 커밋 뒤라 롤백으로 못 살리고 이벤트가 유실될 수 있다. 이벤트를 같은 로컬 트랜잭션에서 outbox 테이블에 INSERT하고 별도 프로세스가 발행하는 Transactional Outbox로 발행 원자성을 확보한다.
